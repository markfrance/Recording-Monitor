...
namespace AE.Net.Mail
{
    ...
    public class ImapClient : TextClient, IMailClient
    {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (data[0] == "*" && data.Length >= 3)
                    {
                        ...
                        {
                            Task.Factory.StartNew(() => _NewMessage.Fire(this, args)); //Fire the event in a task
                        }
                        else if (data[2].Is("EXPUNGE"))
                        {
                            Task.Factory.StartNew(() => _MessageDeleted.Fire(this, args));
                        }
        ...
        public virtual void AppendMail(MailMessage email, string mailbox = null)
        {
            ...
            var command = GetTag() + "APPEND " + (mailbox ?? _SelectedMailbox).QuoteString() + flags + " {" + size + "}";
