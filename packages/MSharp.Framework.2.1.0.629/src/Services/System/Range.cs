    ...
    public class Range<T> where T : IComparable, IComparable<T>
    {
        ...
        public Range(T from, T to)
        {
            if (from.CompareTo(to) > 0)
                throw new ArgumentException("'from' should be smaller than or equal to 'To' value in a range.");
        ...
        /// </summary>
        public bool Lacks(T value, bool includingEdges = true)
        {
            return !Contains(value, includingEdges);
        }
        ...
        {
            return Contains(range.From, includeEdges: true) && Contains(range.To, includeEdges: true);
        }
        ...
        {
            return string.Format(perItemFormat, From) + rangeSeparator + string.Format(perItemFormat, To);
        }
    }
}