...
new DateTime(2099,04,13)};
        ...
        /// <returns></returns>
        private static bool IsAdditionalBankHoliday(DateTime date)
        {
        ...
        /// <returns></returns>
        private static DateTime GetEarlyMayBankHoliday(int year)
        {
        ...
        /// <param name="year">the year to check if in that year there is an exception to the normal bank holiday rule</param>
        /// <returns></returns>
        private static DateTime GetSpringBankHoliday(int year)
        {
        ...
        /// <param name="year">the year to check if in that year there is an exception to the normal bank holiday rule</param>
        /// <returns></returns>
        private static DateTime GetLateSummerBankHoliday(int year)
        {
            ...
            {
                return GetLastWeekdayIn(year, 8, DayOfWeek.Monday);
            }
        }
        ...
        /// <param name="year">the year to check if in that year there is an exception to the normal bank holiday rule</param>
        /// <returns></returns>
        private static DateTime GetBoxingDay(int year)
        {
            var christmasDay = new DateTime(year, 12, 25);
            var result = GetActualHolidayDate(new DateTime(year, 12, 26));
        ...
        private static DateTime GetActualHolidayDate(DateTime originalDay)
        {
        ...
        private static DateTime GetFirstWeekdayIn(int year, int month, DayOfWeek weekday)
        {
        ...
        private static DateTime GetLastWeekdayIn(int year, int month, DayOfWeek weekday)
        {
        ...
        public static bool IsBeforeOrEqualTo(this DateTime date, DateTime otherDate)
        {
        ...
        public static bool IsInSameWeek(this DateTime day, DateTime other)
        {
            ...
            if (other >= beginningOfWeek.AddDays(7)) return false;
        ...
        /// </summary>
        public static bool IsInSameMonth(this DateTime day, DateTime other)
        {
            return day.Month == other.Month && day.Year == other.Year;
        }

        /// <summary>
        /// Gets the number of days in this year.
        /// </summary>
        public static int DaysInYear(this DateTime date) => new DateTime(date.Year, 12, 31).DayOfYear;
        ...
        {
            return date.GetUpcoming(startOfWeek, skipToday: true).AddTicks(-1);
        }
        ...
        {
            return date.ToString("h:mm").TrimEnd(":00") + date.ToString("tt").ToLower();
        }
        ...
        public static string ToTimeDifferenceString(this DateTime date, int precisionParts, bool longForm)
        {
            ...
            else return date.Subtract(now).ToNaturalTime(precisionParts, longForm) + " later";
        }
        ...
        public static DateTime NextWorkingDay(this DateTime date, bool considerEnglishHolidays = true)
        {
            ...
            {
                while (result.IsEnglishHoliday()) result = result.AddDays(1);
            }
            else
            {
                while (result.IsWeekend()) result = result.AddDays(1);
            }
        ...
        public static TimeSpan CalculateTotalWorkingHours(this DateTime date, TimeSpan period, TimeSpan workingStartTime, TimeSpan workingEndTime, bool considerEnglishBankHolidays = true)
        {
            ...
            if (workingStartTime < TimeSpan.Zero || workingStartTime >= TimeSpan.FromHours(24))
                throw new ArgumentException("fromTime should be greater than or equal to 0, and smaller than 24.");

            if (workingEndTime <= TimeSpan.Zero || workingEndTime > TimeSpan.FromHours(24))
        ...
        {
            return value < SqlDateTime.MinValue.Value ? SqlDateTime.MinValue.Value : value;
        }
        ...
        {
            return date.GetBeginningOfQuarter().AddMonths(4).GetBeginningOfQuarter().AddSeconds(-1d);
        }
        ...
        {
            return date.GetBeginningOfMonth().AddMonths(1).AddSeconds(-1);
        }
        ...
        {
            return new DateTime(date.Year + 1, 1, 1).AddSeconds(-1);
        }
        ...
        public static DateTime AddWeeks(this DateTime date, int numberofWeeks)
        {
            ...
            return date.AddDays(7 * numberofWeeks);
        }
